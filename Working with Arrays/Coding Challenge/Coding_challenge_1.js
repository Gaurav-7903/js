...
const checkDogs = function(dogsJulia , dogsKate){
    const correctedDogsJulia = dogsJulia.slice(1, -1);// removing the last two cats
    const allDogs = dogsKate.concat(correctedDogsJulia);

    allDogs.forEach((dog, index) => {
        console.log(`Dog number ${index + 1} is ${dog < 3? 'a puppy' : `an adult  and is ${dog} years old`}`);
    });

    return allDogs;
}

checkDogs(data1.dogsJulia, data1.dogsKate);
// checkDogs(data2.dogsJulia , data2.dogsKate);