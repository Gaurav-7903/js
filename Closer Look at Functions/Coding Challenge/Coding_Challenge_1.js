...
Bonus: Use the displayResults method to display the 2 arrays in the test data. Use both the array and the string option. Do not put the arrays in the poll object! So what should the this keyword look like in this situation? Test data for bonus:
...
const poll = {
  ...
  registerNewAnswer() {
        ...
        // register new answer
        if (!isNaN(UserAnswer) && UserAnswer >= 0 && UserAnswer < this.options.length) {
          this.answers[UserAnswer]++;
        }else{
            alert("Invalid input. Please enter a number between 0 and " + (this.options.length - 1));
        }
